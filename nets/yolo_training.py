...
class YOLOLoss(nn.Module):
    ...
    def find_3_positive(self, predictions, targets):
        ...
        for i in range(len(predictions)):
            ...
            indices.append((b, a, torch.round(gj.float().clamp_(0, gain[3] - 1)).to(torch.int64), torch.round(gi.float().clamp_(0, gain[2] - 1)).to(torch.int64)))
            # indices.append((b, a, gj.clamp_(0, gain[3] - 1), gi.clamp_(0, gain[2] - 1)))  # image, anchor, grid indices  11111111
