...
class MainWindow(QWidget):
    ...
    def set_ui(self):
        ...
        right_right_panel = self._create_right_right_panel()
    ...
    def _create_right_right_panel(self):
        """创建右侧区域（状态信息和轨迹显示）"""
        frame = QFrame()
        frame.setObjectName("rightrightFrame")
        layout = QVBoxLayout(frame)
        layout.setSpacing(30)
    ...
    def record_state(self):
        if self.record_state_flag:
            ...
            # 主线程中更新初始状态
            self.update_status_text("Recording stopped. Evaluating flight...\n")
            self.update_status_text("Please wait for a moment...\n")
            self.update_status_text(f"{log_data}")

            # 在后台线程中处理评估逻辑
            def process_evaluation():
                try:
                    result = evaluate_all_flight(log_data)
                    content = now_time + "\n" + result + "\n"
                    # 发射信号传递评估结果
                    self.evaluation_completed.emit(content)
                except Exception as e:
                    print(f"Error during evaluation: {e}")
                    error_message = f"Error during evaluation: {e}"
                    self.evaluation_completed.emit(error_message)

            # 启动后台线程
            Thread(target=process_evaluation).start()
        ...
        if frame.shape == (self.fpv_uav_resolution_ratio[1], self.fpv_uav_resolution_ratio[0], 3):
            ...
            if self.capture_flag:
                ...
                for key, val in capture_dict.items():
                    ...
                    filename = "capture_" + key + "_" + str(
                        datetime.now().strftime('date_%m_%d_%H_%M_%S')) + "_" + ".png"
