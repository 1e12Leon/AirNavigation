...
class MainWindow(QWidget):
    ...
    def _init_command_system(self):
        ...
        # 在这里填入api AIzaSyDFNNDLUzz_wwBFPTs4m0jiV7SWfO5JTAc
        # AIzaSyAm6AFEV7K4mBLwsVBBw41QAy-pwE7fPIc
        # AIzaSyBGf3Gg7tY40Vs1d21sItTJ4foVEH-lCHA
    ...
    def _create_left_control_panel(self):
        ...
        # 连接按钮
        self.btn_connect = self._create_button("连接", self.connect)

        # 拍照按钮
        self.btn_capture = self._create_button("拍照", self.capture, enabled=False)

        # 录像按钮
        self.btn_record_video = self._create_button("开始录制", self.record_video, enabled=False)

        # 工作模式按钮
        self.btn_change_work_mode = self._create_button("切换模式", self.change_work_mode, enabled=False)

        # BoT-SORT 输入框
        self.text_input_botsort = QLineEdit(self)
        self.text_input_botsort.setPlaceholderText("输入 BoT-SORT target ID")
        ...
        # 其他功能按钮
        self.btn_change_weather = self._create_button("切换天气", self.change_weather, enabled=False)
        self.btn_change_uav = self._create_button("切换无人机", self.change_uav, enabled=False)
        self.btn_change_map = self._create_button("切换地图", self.change_map, enabled=False)
...
飞行速度：{speed:.2f}m/s"""
    ...
    def _create_medium_panel(self):
        ...
        self.btn_instructions = QPushButton("使用说明")
    ...
    def toggle_monitoring(self):
        if self.monitoring_flag:
            ...
            self.btn_toggle_monitoring.setText("开始监控")
            if self.monitoring_thread:
                self.monitoring_thread.stop()
                #self.monitoring_thread.wait()
            self.update_status_text("暂停监控")
        else:
            # 开始监控
            self.monitoring_flag = True
            self.btn_toggle_monitoring.setText("暂停监控")
    ...
    def _create_button(self, text, callback, enabled=True):
        ...
        btn.setStyleSheet(self._get_style_sheet())
        return btn
    ...
    def _create_central_area(self):
        ...
        self.cmd_input.setPlaceholderText("输入您的命令：")
    ...
    def _create_right_panel(self):
        ...
        self.label_status.setAlignment(Qt.AlignLeft | Qt.AlignTop)  # 左上对齐
        self.label_status.setWordWrap(True)  # 启用自动换行
        ...
        # 轨迹显示区域
        trajectory_group = QGroupBox("实时轨迹")
    ...
    def _get_style_sheet(self):
        ...
        return """
            QFrame {
                ...
                border: 1px solid rgb(200, 200, 200); /* 浅灰色边框 */
                border-radius: 5px;
                margin: 2px;
            ...
            QGroupBox {
                ...
                background-color: rgb(245, 245, 245); /* 浅灰色背景 */
                color: rgb(0, 0, 0);
            }
            ...
            QPushButton {
                color: rgb(0, 0, 0);
                background-color: rgb(200, 200, 200); /* 浅灰色按钮 */
                border: 1px solid rgb(200, 200, 200);
                border-radius: 5px;
                padding: 5px 10px;
                margin: 2px;
            ...
            QWidget {
                background-color: rgb(255, 255, 255); /* 主背景白色 */
                border-radius: 5px;
            }
        """
    ...
    def record_state(self):
        if self.record_state_flag:
            ...
            self.btn_record_state.setText("开始记录")
    ...
    def project_introduce(self):
        msgbox = QMessageBox(QMessageBox.Information,
                             "帮助:\n"
                             "这个项目的核心算法使用YOLOv7\n"
                             "无人机的模拟平台使用Airsim + UE4.\n"
                             "UI使用PyQt5, LLM评估使用Genimi1.5",
                             QMessageBox.Ok, self)
        msgbox.show()
    ...
    def update_label_status(self):
        ...
        self.label_status.setText("无人机位置：" + str((x, y, z)) +
                                  "\n姿势调整次数：" + str(self.adjustment_cnt) +
                                  "\n天气状态：" + weather + ", " + str(val) +
                                  "\n无人机工作模式：" + work_mode +
                                  "\n定位目标数量:" + str(target_nums) +
                                  "\n最近已定位目标位置:" + str(latest_target_location))
